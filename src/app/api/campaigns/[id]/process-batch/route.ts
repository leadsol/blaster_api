...
import { BATCH_SIZE, DEVICE_BASE_LIMIT as BASE_MESSAGES_PER_DAY_PER_DEVICE, DEVICE_VARIATION_BONUS as EXEMPT_MESSAGES_PER_VARIATION } from '@/lib/constants'
...
async function handler(
  ...
  try {
    ...
    if (campaignError || !campaign) {
      logger.error('[PROCESS-BATCH] Campaign not found:', campaignId)
      return NextResponse.json({ error: 'Campaign not found' }, { status: 404 })
    }

    // Check if campaign should be running
    if (!['running', 'draft', 'scheduled'].includes(campaign.status)) {
      logger.debug(`[PROCESS-BATCH] Campaign ${campaignId} not in runnable state (${campaign.status})`)
    ...
    if (campaign.is_active === false) {
      logger.debug(`[PROCESS-BATCH] Campaign ${campaignId} is deactivated (is_active: false)`)
    ...
    if (effectiveCount >= baseLimit) {
      logger.debug(`[PROCESS-BATCH] Daily limit reached (sent: ${messagesTodayCount}, exempt: ${exemptMessages}, effective: ${effectiveCount}/${baseLimit})`)
    ...
    if (messagesError) {
      logger.error('[PROCESS-BATCH] Error fetching messages:', messagesError)
      return NextResponse.json({ error: 'Failed to fetch messages' }, { status: 500 })
    }

    if (!pendingMessages || pendingMessages.length === 0) {
      // No more pending messages - campaign complete
      logger.debug(`[PROCESS-BATCH] Campaign ${campaignId} completed - no pending messages`)
    ...
    logger.debug(`[PROCESS-BATCH] Total sent messages so far: ${sentSoFar}`)
    ...
    for (let i = 0; i < pendingMessages.length; i++) {
      ...
      if (i === 0) {
        ...
        if (spacing < MIN_DELAY) {
          ...
          logger.debug(`[PROCESS-BATCH] Generated random spacing: ${spacing}s (original was ${currentDelay - prevDelay}s)`)
        }
      ...
      logger.debug(`[PROCESS-BATCH] Scheduling message ${message.id} with ${delayFromNow}s total delay from now (scheduled_delay: ${currentDelay}s)`)
      ...
      if (isQStashConfigured()) {
        ...
        setTimeout(async () => {
          try {
            logger.debug(`[FALLBACK] Sending message ${message.id} now`)
            ...
            logger.error(`[FALLBACK] Failed to send message ${message.id}:`, err)
          }
        }, delayFromNow * 1000)

        scheduledCount++
        logger.debug(`[FALLBACK] Scheduled message ${message.id} with ${delayFromNow}s delay (setTimeout)`)
      }
    }
    ...
    if (remaining > 0) {
      ...
      logger.debug(`[PROCESS-BATCH] Scheduling next batch in ${nextBatchDelay}s (${remaining} messages remaining)`)
      ...
      if (isQStashConfigured()) {
        ...
        setTimeout(async () => {
          try {
            logger.debug(`[FALLBACK] Triggering next batch for campaign ${campaignId}`)
            ...
            logger.error(`[FALLBACK] Failed to trigger next batch:`, err)
          }
        }, nextBatchDelay * 1000)

        logger.debug(`[FALLBACK] Scheduled next batch in ${nextBatchDelay}s (setTimeout)`)
      }
    } else {
      ...
      logger.debug(`[PROCESS-BATCH] Scheduling final check in ${finalCheckDelay}s to mark campaign as completed`)
      ...
      if (isQStashConfigured()) {
        ...
        setTimeout(async () => {
          try {
            logger.debug(`[FALLBACK] Final check for campaign ${campaignId}`)
            await fetch(`${appUrl}/api/campaigns/${campaignId}/process-batch`, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
            ...
            logger.error(`[FALLBACK] Failed to trigger final check:`, err)
          }
        }, finalCheckDelay * 1000)

        logger.debug(`[FALLBACK] Scheduled final check in ${finalCheckDelay}s (setTimeout)`)
      }
    }
